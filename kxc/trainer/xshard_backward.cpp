...
int main(int argc, char** argv) {
    ...
    // Renumber to 0-based local indices. xshard_adapt uses seq as a direct array
    // index into the gradient file, so the global seq numbers from the model must
    // not be preserved here.
    for (size_t i = 0; i < out.size(); ++i) out[i].rec.seq = static_cast<int>(i);
